...
def minmax_size(img: Image, max_dimensions: Tuple[int, int] = None, min_dimensions: Tuple[int, int] = None) -> Image:
    ...
    if max_dimensions is not None:
        ...
        if any([r > 1 for r in ratios]):
            ...
            img = img.resize(size.astype(int), Image.Resampling.BILINEAR)
        ...
        if (self.image_resizer is not None and not self.args.no_resize) and resize:
            with torch.no_grad():
                ...
                for _ in range(10):
                    ...
                    img = pad(minmax_size(input_image.resize((w, h), Image.BILINEAR if r > 1 else Image.Resampling.LANCZOS), self.args.max_dimensions, self.args.min_dimensions))
...
def main():
    ...
    with in_model_path():
        ...
        '''instructions = input('Nhấn phím "enter" để in biểu thức. ')
    ...
    Settings:
        ...
        img = None
            #if file:
        img = Image.open("E:/ctf/ke/ocr/ocr/bin/Debug/scan.jpg")
            #else:
            #    try:
